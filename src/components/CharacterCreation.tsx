  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 py-10 px-4">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <section className="space-y-2">
          <label className="text-sm text-slate-400">עיר מגורים</label>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {CITIES.map((c) => (
              <button
                key={c.id}
                onClick={() => setCity(c.id)}
                className={`px-3 py-2 rounded-lg border text-sm text-right ${city === c.id ? 'bg-sky-500 text-slate-950 border-sky-500 font-bold' : 'border-slate-700 hover:border-slate-500'}`}
              >
                <div className="font-semibold">{c.label}</div>
                <div className="text-xs opacity-80">{c.vibe}</div>
              </button>
            ))}
          </div>
        </section>
